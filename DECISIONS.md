...
## 2026-08-06 - The Blocker's Own Success Hides The Slot, So The Gap Is Claimed Instead

**Decision**: A collapsed ad wrapper promotes to the ancestor holding the empty height, and that ancestor is claimed with reason `reserved ad gap`. New predicates `isBlockedAdResidue`, `holdsNothingButResidue`, `findReservedAdGap`, and `isReservedAdGap` in `src/scanner.js`; the last is also an escape from `isTooLargeForMvp`.

**Why**: this is the last of the seven live caption survivors, six of them on The Sun, deferred since 2026-07-18. The cause is ours. Our DNR layer blocks the creative before it paints, so `div.advert-wrapper.advert-section-container` collapses to the 36px of its own caption — four pixels under the 40px floor in `isVisibleRect` — and becomes invisible to the scanner. Its `div.module--1-billboard` ancestor still holds the 330px the creative was going to fill, but carries none of the ad's identifiers, and `hasAdLabel` alone is not enough: `getMatchReason` requires a label to come with `isSmallContainer` (height <= 320; the module is 330) or `isSidebarElement` (false; it is a full-width module in the article flow). Neither the wrapper nor the module was reachable, so the reader was left with a captioned blank.

Widening a size threshold or adding `billboard` to a regex would have claimed the module by accident rather than on the evidence. The evidence is that a wrapper naming itself an ad collapsed to nothing, which is a fact about our own blocking, not a guess about the site.

**Consequences**:
- `holdsNothingButResidue` is what keeps this off page-level containers. A survey of the 21 regression sites found residue on three: The Sun (parent 330px, text length 13 — a genuine gap), NY Post (`div.site-content`, text length 33651) and TMZ (`body`, text length 453014). The two large ones are excluded by having content in them, and by the stop selector besides. Confirmed live: zero `reserved ad gap` claims on either site.
- The 21-site run moved three sites. The Sun goes 1 card to 7 and 6 captions to 0, all six new claims being `div.module--1-billboard` at 1583x330. Daily Mail (14 to 16) and TMZ (13 to 11) are page variance — probing both showed no `reserved ad gap` among their claims. The other 18 sites are identical.
- Presence still decides card-versus-collapse, so the gap becomes a Tide card at Full Ambient and collapses out of flow at Clean. Both are asserted in `scripts/test-extension.mjs`.
...
## 2026-08-06 - A Preview Of The Card Is Held To The Card

**Decision**: the welcome page's `.note-card` and the options page's `.note-specimen` are bound to `src/content.css` and the `replacer.js` type ramps, not merely styled to resemble them. Both are pinned to the light tone whatever theme the surrounding page is in. The extension's own chrome carries no ambient motion, and public copy no longer describes the deleted field.

**Why**: both pages exist to show a stranger what the extension does, and both were still drawing the gradient, drifting swell, and ember orb that the flat-card port removed. The welcome page is the first thing a new install opens; it was advertising a product that no longer ships. A preview that is not the thing is worse than no preview, because it is believed.

**Consequences**:
- Surface `#e6ebe3`, ink `#20312a`, ring `inset 0 0 0 1px rgba(32,49,42,.12)`, radius 12px, `align-items: center`, and left-set long notes are the light-tone values from `src/content.css`, verified against a live card from the `card-matrix` fixture on computed styles rather than by eye.
- Leading and tracking come off `cardLineHeight` and `cardTracking`. The first pass invented `1.35` and `-0.03em`; neither is a value those ramps can produce, and `-0.03em` is tighter than the `-0.022em` floor. Inventing plausible-looking numbers is the specific way a preview drifts, because nothing about them looks wrong.
- The welcome clamp floor moved from 19px to 20px so the whole range sits in one bucket of both ramps and a single pair of values stays true at every viewport width.
- Padding is the real `clamp(min(w, h) * 0.1, 10, 48)`. At `aspect-ratio: 6/5` the height is the short side, so `h/10` is `w/12`, and CSS percentage padding resolves against inline size — `8.333%` tracks the formula across every column width this page produces, and the 10/48 bounds are never reached at those widths.
- `max-width: 15ch` was removed. The card's measure cap is `min(100%, 30em)`, which at these sizes never binds; the invented cap wrapped the note a full line earlier than the real card would.
- Font size is the one value a static preview cannot carry, because the real one is solved per slot by `fitCardText`. This is stated in the CSS rather than papered over.
- Both previews are a *third* hand-synced copy of the card's look, after `src/content.css` and `SHADOW_ROOT_STYLE_TEXT` in `src/shared.js`. Generating the copies from `content.css` at build time is now worth more than it was, and remains unbuilt.
- The 45s popup and 48s welcome drift animations are gone. Each held a compositor layer alive for the life of the surface to move a blurred highlight nobody was looking at, and each was the ambient idea the card itself had already dropped. The still highlight stays.
