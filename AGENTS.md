...
- A rule change also needs `npm run test:dnr-match`, which `npm run check` does not run because it needs a browser. Read its whole output rather than grepping for the cases you added: a gate that drops too much shows up as a *pre-existing* case flipping, and grepping for your own case names hides exactly that. Learned 2026-09-09, when a too-broad exception gate broke the assertion that `gpt.js` stays allowed and only CI noticed.
